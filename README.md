...
```shell
docker run -it -d --init --name glm-free-api -p 8000:8000 -e TZ=Asia/Shanghai -e TOKENS="token1|token2" -e KEY="your_api_key" akashrajpuroh1t/glm-free-api-fix
```

> **注意**：如果你设置了 `KEY`，在客户端调用时只需要在 Authorization 中填入 `Bearer your_api_key` 即可，服务端会自动从 `TOKENS` 中随机轮询。
...
*   **`TOKENS`**: (可选) 预设的 `chatglm_refresh_token` 列表，使用 `|` 分隔。
*   **`KEY`**: (可选) 自定义服务端 API Key。如果设置了此项，客户端请求时在 `Authorization` 头部填入此 Key，服务端将自动从 `TOKENS` 列表中轮询分配 Token。
